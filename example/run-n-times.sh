...
#    done
    ...
    for i in $(seq 1 1 $N)
    do
        ./train-model.sh $d 5 0 $SHUFFLE false data/train.en.lm 2> /dev/null | tail -n2 >> train-plus-bigram.$d
        ./test-model.sh $d 10 0 false data/train.en.lm 2> /dev/null | tail -n1 >> test-plus-bigram.$d
    done
