...
echo "../src/bin/train-lader -cost 1e-3 -attach_null right -feature_profile \"seq=dict=output/train.en-ja.pt,LL%SL%ET,RR%SR%ET,LR%LR%ET,RL%RL%ET,O%SL%SR%ET,I%LR%RL%ET,Q%SQE0%ET,Q0%SQ#00%ET,Q1%SQ#01%ET,Q2%SQ#02%ET,CL%CL%ET,B%SB%ET,A%SA%ET,N%SN%ET,BIAS%ET|seq=LL%SL%ET,RR%SR%ET,LR%LR%ET,RL%RL%ET,B%SB%ET,A%SA%ET,O%SL%SR%ET,I%LR%RL%ET|seq=LL%SL%ET,RR%SR%ET,LR%LR%ET,RL%RL%ET,B%SB%ET,A%SA%ET,O%SL%SR%ET,I%LR%RL%ET|cfg=LP%LP%ET,RP%RP%ET,SP%SP%ET,TP%SP%LP%RP%ET\" -non_local_feature_profile \"non-local=TL%SL,TR%SR,TLR%LR,TRL%RL,TO%SL%SR,TI%LR%RL|non-local=TL%SL,TR%SR,TLR%LR,TRL%RL,TO%SL%SR,TI%LR%RL|non-local=TL%SL,TR%SR,TLR%LR,TRL%RL,TO%SL%SR,TI%LR%RL\" -iterations 500 -shuffle $SHUFFLE -samples $SAMPLES -verbose $VERBOSE -gap-size $GAP -model_out output/train-g$GAP.mod -source_in output/train.en.annot -align_in data/train.en-ja.align"
../src/bin/train-lader -cost 1e-3 -attach_null right -feature_profile "seq=dict=output/train.en-ja.pt,LL%SL%ET,RR%SR%ET,LR%LR%ET,RL%RL%ET,O%SL%SR%ET,I%LR%RL%ET,Q%SQE0%ET,Q0%SQ#00%ET,Q1%SQ#01%ET,Q2%SQ#02%ET,CL%CL%ET,B%SB%ET,A%SA%ET,N%SN%ET,BIAS%ET|seq=LL%SL%ET,RR%SR%ET,LR%LR%ET,RL%RL%ET,B%SB%ET,A%SA%ET,O%SL%SR%ET,I%LR%RL%ET|seq=LL%SL%ET,RR%SR%ET,LR%LR%ET,RL%RL%ET,B%SB%ET,A%SA%ET,O%SL%SR%ET,I%LR%RL%ET|cfg=LP%LP%ET,RP%RP%ET,SP%SP%ET,TP%SP%LP%RP%ET" -non_local_feature_profile "non-local=TL%SL,TR%SR,TLR%LR,TRL%RL,TO%SL%SR,TI%LR%RL|non-local=TL%SL,TR%SR,TLR%LR,TRL%RL,TO%SL%SR,TI%LR%RL|non-local=TL%SL,TR%SR,TLR%LR,TRL%RL,TO%SL%SR,TI%LR%RL" -iterations 500 -shuffle $SHUFFLE -samples $SAMPLES -verbose $VERBOSE -gap-size $GAP -model_out output/train-g$GAP.mod -source_in output/train.en.annot -align_in data/train.en-ja.align
