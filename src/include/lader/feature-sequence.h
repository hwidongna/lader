...
public:
    ...
    //  %[SLR][AB] : Left or Right-most boundary word of a span
    ...
    //  %C[DBL] :        Difference (absolute value or indicator -1/0/+1) in words in two spans as string
    //  %C[DB]# :        Difference (absolute value or indicator -1/0/+1) in words in two spans as value
...
protected:
    double GetFeatureValue(const FeatureDataSequence & sent,
			const HyperEdge & edge, const std::string & type);
	std::string GetFeatureString(const FeatureDataSequence & sent,
			const HyperEdge & edge, const std::string & type);
